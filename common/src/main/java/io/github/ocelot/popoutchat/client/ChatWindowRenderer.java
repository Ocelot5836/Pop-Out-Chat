...
import io.github.ocelot.popoutchat.client.util.ScrollHandler;
...
{
    private static final List<FormattedCharSequence> RENDER_LINES = new LinkedList<>();
    ...
    {
        if (colorBuffer == 0)
            colorBuffer = GlStateManager._genTexture();
        if (depthBuffer == 0)
    ...
    public static void attach()
    {
        ...
        bind();
        glFramebufferTexture2D(GlConst.GL_FRAMEBUFFER, GlConst.GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorBuffer, 0);
        glFramebufferRenderbuffer(GlConst.GL_FRAMEBUFFER, GlConst.GL_DEPTH_ATTACHMENT, GlConst.GL_RENDERBUFFER, depthBuffer);
    ...
    public static void updateText()
    {
        ...
        ((ChatComponentAccessor)mc.gui.getChat()).getAllMessages().stream().flatMap(line ->
        {
            List<FormattedCharSequence> processors = ComponentRenderUtils.wrapComponents(line.getMessage(), ChatWindow.getScaledWidth() - 4 * ChatWindow.getGuiScale(), Minecraft.getInstance().font);
        ...
        SCROLL_HANDLER.setHeight((int) (RENDER_LINES.size() * 9.0D * (mc.options.chatLineSpacing + 1.0D)) - 9);
    }
    ...
    public static void free()
    {
        ...
        if (colorBuffer != 0)
        {
            GlStateManager._deleteTexture(colorBuffer);
            colorBuffer = 0;
        }
        if (depthBuffer != 0)
        {
    ...
    public static void tick()
    {
        if (colorBuffer != 0 && depthBuffer != 0)
        {
            ...
            if (options.chatLineSpacing != lastChatLineSpacing)
            {
                SCROLL_HANDLER.setHeight((int) (RENDER_LINES.size() * 9.0D * (options.chatLineSpacing + 1.0D)) - 9);
                lastChatLineSpacing = options.chatLineSpacing;
            }
    ...
    public static void render(PoseStack matrixStack, int scaledWidth, int scaledHeight, int mouseX, int mouseY, float partialTicks)
    {
        Gui.fill(matrixStack, 0, 0, scaledWidth, scaledHeight, 0xFFC6C6C6);
        ...
        double d0 = Minecraft.getInstance().options.chatScale;

        if (!((ChatComponentAccessor)gui).getChatQueue().isEmpty())
        {
            long now = System.currentTimeMillis();
            if (now - ((ChatComponentAccessor) gui).getLastMessage() >= mc.options.chatDelay * 1000.0D)
            {
                gui.addMessage(((ChatComponentAccessor)gui).getChatQueue().remove());
                ((ChatComponentAccessor) gui).setLastMessage(now);
            }
        }
        ...
        if (j > 0)
        {
            int k = Mth.ceil((double) scaledWidth / d0);
            RenderSystem.pushMatrix();
            RenderSystem.translatef(2.0F, 8.0F, 0.0F);
            RenderSystem.scaled(d0, d0, 1.0D);
            double d1 = mc.options.chatOpacity * (double) 0.9F + (double) 0.1F;
            double d2 = mc.options.textBackgroundOpacity;
            double d3 = 9.0D * (mc.options.chatLineSpacing + 1.0D);
            double d4 = -8.0D * (mc.options.chatLineSpacing + 1.0D) + 4.0D * mc.options.chatLineSpacing;
            ...
            {
                FormattedCharSequence chatline = RENDER_LINES.get((int) (i1 + scrollPos / d3));
                if (chatline != null)
                {
                    ...
                    if (l1 > 3)
                    {
                        ...
                        Gui.fill(matrixStack, -2, (int) (d6 - d3), k + 4, (int) d6, i2 << 24);
                        RenderSystem.enableBlend();
                        matrixStack.translate(0.0D, 0.0D, 10.0D);
                        mc.font.drawShadow(matrixStack, chatline, 0.0F, (float) ((int) (d6 + d4)), 16777215 + (l1 << 24));
            ...
            if (!((ChatComponentAccessor)mc.gui.getChat()).getChatQueue().isEmpty())
            {
                int k2 = (int) (128.0D * d1);
                int i3 = (int) (255.0D * d2);
                matrixStack.pushPose();
                matrixStack.translate(0.0D, 0.0D, 50.0D);
                Gui.fill(matrixStack, -2, 0, k + 4, 9, i3 << 24);
                RenderSystem.enableBlend();
                matrixStack.translate(0.0D, 0.0D, 50.0D);
                mc.font.drawShadow(matrixStack, new TranslatableComponent("chat.queue", ((ChatComponentAccessor)mc.gui.getChat()).getChatQueue().size()), 0.0F, 1.0F, 16777215 + (k2 << 24));
                matrixStack.popPose();
                RenderSystem.disableAlphaTest();
                RenderSystem.disableBlend();
            }
    ...
    public static Style getHoveredComponent(double mouseX, double mouseY, float partialTicks)
    {
        ...
        d0 = Mth.floor(d0 / Minecraft.getInstance().options.chatScale);
        d1 = Mth.floor(d1 / (Minecraft.getInstance().options.chatScale * (mc.options.chatLineSpacing + 1.0D)));
        if (!(d0 < 0.0D) && !(d1 < 0.0D))
        {
            int i = Math.min((int) Math.ceil((ChatWindow.getScaledHeight() - 48) / 9.0) + 2, RENDER_LINES.size());
            if (d0 <= (double) Mth.floor((double) ChatWindow.getScaledWidth() / Minecraft.getInstance().options.chatScale) && d1 < (double) (9 * i + i))
            {
                int j = (int) ((d1 + scrollPos) / 9.0D);
                if (j >= 0 && j < RENDER_LINES.size())
                {
                    return mc.font.getSplitter().componentStyleAtWidth(RENDER_LINES.get(j), (int) d0);
                }
            }
        }
        return null;
